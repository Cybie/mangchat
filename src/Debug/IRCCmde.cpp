...
void IRCCmd::change_lang(_CDATA *CD)
{
	std::string* _PARAMS = getArray(CD->PARAMS, 1);
	// Added for 
	sIRC.localeInfo = _wsetlocale(LC_ALL, std::wstring(_PARAMS[0].begin(), _PARAMS[0].end()).c_str());
	if(sIRC.localeInfo == NULL) 
	{
		fprintf(stderr, "[ERROR] Can't set the specified locale! "
              "Check LANG, LC_CTYPE, LC_ALL.\n");
	    sIRC.Send_IRC_Channel(ChanOrPM(CD), MakeMsg(" 13Language dident work. sent this option [%s]: !", _PARAMS[0].c_str()), true);
	}
	else
	{
		sIRC.localeInfo_start = (char *) _PARAMS[0].c_str();   
		printf("%ls -> [%s]\n", L"We know have support for charset", sIRC.localeInfo);
		sIRC.Send_IRC_Channel(ChanOrPM(CD), MakeMsg(" 13Language [%s] choosen. sent this option [%s]: !", (const char *) sIRC.localeInfo, _PARAMS[0].c_str()), true);
	}
}
